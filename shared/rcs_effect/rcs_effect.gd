...
func calc_fx_intensity(cmd_rotate, cmd_translate):
	...
	fx_intensity = lerp(fx_intensity, float(clamp((vec_result.x + vec_result.y + vec_result.z), 0, 1)), 0.05)


# Called every frame. 'delta' is the elapsed time since the previous frame.
func _process(delta):
	if fx_intensity > 0.01:
		$GPUParticles3D.visible = true
		$GPUParticles3D.scale = Vector3.ONE * 0.25 * fx_intensity
		$OmniLight3D.light_energy = 0.05 * fx_intensity
	else:
