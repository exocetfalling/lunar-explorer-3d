...
@export var param_tuning_active = false
...
# Clamp max/min outputs
@export var param_clamp_active: bool = false
@export var param_clamp_max: float = 1.00
@export var param_clamp_min: float = -1.00
...
#		time_delta = 0.0167
	...
	# Clamp min/max
	if param_clamp_active:
		output_total = clamp(output_total, param_clamp_min, param_clamp_max)
